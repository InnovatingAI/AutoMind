...
class Coder(ABC):
    ...
    def _prompt_environment(self):
        pkgs = [
            "bayesian-optimization==1.5.1", "lightgbm==4.5.0", "matplotlib==3.9.2",
            "numpy==1.26.2", "optuna==4.0.0", "pandas==2.1.4", "scikit-learn==1.2.2",
            "scipy==1.11.4", "seaborn==0.13.2", "statsmodels==0.14.4", "timm==0.9.7",
            "torch==2.2.0", "torchvision==0.17.0", "torch-geometric==2.6.1",
            "transformers==4.44.2", "xgboost==2.1.3",
        ]
        random.shuffle(pkgs)
    ...
    @property
    def _common_code_guidelines(self):
        return [
            ...
            "Be aweare of the execution time limit of **9 HOURS**."
        ]

    @property
    def _diff_format_instructions(self):
        return [
    ...
    def _apply_diff_and_validate(self, original_code: str, diff_blocks: list, interpreter=None):
        """Apply diff blocks to code and validate syntax"""
        modified_code = apply_diff_to_code(original_code, diff_blocks)
        
        try:
            ast.parse(modified_code)
            logger.info(f"Diff-based generation succeeded. Applied {len(diff_blocks)} changes.")
            
            if interpreter:
                exec_result = interpreter.run(modified_code)
                return modified_code, exec_result
            else:
                return modified_code, None
        except Exception as e:
            logger.info(f"Diff-based generation failed: {e}")
            return None, None
...
class OneShotCoder(Coder):
    ...
    def _prompt_code_guideline(self):
        ...
        return {"Code guideline": guidelines + self._common_code_guidelines}
    ...
    def prompt_integration(
        ...
        if error_code:
            prompt["Last Try Code"] = wrap_code(error_code)
        if error_output:
            prompt["Last Try Execution Output"] = trim_long_string(str(error_output))
    ...
    def _get_debug_diff_prompt(self, plan: str, data_analysis: Optional[str], based_code: str):
        """Generate prompt for diff-based debugging"""
        original_code = extract_code(based_code) if "```" in based_code else based_code
        
        prompt = {
            "Introduction": "You are debugging a failed ML solution. Use precise SEARCH/REPLACE format to fix errors.",
            "Task Description": self.task_desc,
            "Improved Solution Plan": plan,
            "Failed Code": wrap_code(original_code),
            "Instructions": {
                "Diff Format": self._diff_format_instructions,
                **self._prompt_code_guideline
            }
        }
        ...
        prompt["Instructions"].update(self._prompt_environment)
        if self.human_guideline:
            prompt["Instructions"].update({"Human Guideline": self.human_guideline})
        
        return prompt

    def _step_with_diff(self, plan: str, data_analysis: Optional[str], based_code: str, retry: int):
        """Use diff format for debugging existing code"""
        logger.info("Agent is using one-shot precise debug")

        prompt = self._get_debug_diff_prompt(plan, data_analysis, based_code)
        ...
        for _ in range(retry):
            ...
            if diff_blocks:
                original_code = extract_code(based_code) if "```" in based_code else based_code
                result = self._apply_diff_and_validate(original_code, diff_blocks, self.interpreter)

                logger.info(f"=== MODIFIED CODE AFTER DIFF ===")
                logger.info(f"{result}")
                logger.info(f"=== END OF MODIFIED CODE ===")

                if result[0] is not None:
                    return result
                else:
                    # Log the execution failure for diff-based debugging
                    if result[1] and result[1].exc_type:
                        logger.info(f"Code testing failed with diff-based debugging with the following error: \n{result[1].term_out}\n")
            else:
                # Fallback to regular generation
                logger.info("No valid diff blocks found, falling back to regular generation")
                code = extract_code(response)
                if code:
                    try:
                        ast.parse(code)
                        logger.info("Fallback code generation succeeded.")
                        exec_result = self.interpreter.run(code)
                        if exec_result.exc_type:
                            logger.info(f"Code testing failed with fallback generation with the following error: \n{exec_result.term_out}\n")
                        else:
                            return code, exec_result
                    except Exception as e:
                        logger.info(f"Fallback code generation failed: {e}")
        
        logger.error("Diff-based debug failed after retries")
        return None, None
    ...
    def step(
        ...
        if is_debug and based_code:
            return self._step_with_diff(plan, data_analysis, based_code, retry)
        ...
        for _ in range(retry):
            ...
            if code:
                try:
                    ast.parse(code)
                    logger.info(f"Code generation successed. Implemented code: \n\n{code}\n\n")
                    logger.info(f"Agent is executing the code")
                    exec_result = self.interpreter.run(code)
                    # Check if the code has raised an exception
                    if exec_result.exc_type:
                        logger.info(f"Code testing failed with the following error: \n{exec_result.term_out}\n")
                        last_failed_code = code
                        last_error_output = exec_result.term_out
                        continue
                    else:
                        return code, exec_result
                except Exception as e:
                    ...
                    last_failed_code = code
                    last_error_output = f"Syntax or parse check failed: {e}"
                    continue
            else:
                logger.info("Code generation failed, retrying ...")
                last_error_output = "Failed to extract a valid code block from the model response."
...
class StepByStepCoder(Coder):
    ...
    def plan_decomposition(self, plan: str, retry: int = 3):
        ...
        for _ in range(retry):
            ...
            if json_obj and "decomposed steps" in json_obj:
                ...
                logger.info(f"JSON extraction successed. Decomposed steps: \n\n{merged_steps}\n\n")
                return merged_steps
            else:
    ...
    def prompt_integration(
        ...
        if last_try_step_code and last_try_exec_output:
            removeprefix_step_code = last_try_step_code.removeprefix(f"{prev_steps_code}\n")
    ...
    def _get_diff_prompt(self, step_plan: dict, prev_steps_code: str, 
                        last_try_step_code: str, last_try_exec_output: str):
        """Generate prompt for diff-based debugging"""
        removeprefix_step_code = last_try_step_code.removeprefix(f"{prev_steps_code}\n") if prev_steps_code else last_try_step_code
        
        prompt = {
            "Introduction": "You are debugging a failed ML code step. Use precise SEARCH/REPLACE format to fix errors.",
            # "Task Description": self.task_desc,
            "Current Step": str(step_plan),
            "Failed Code": wrap_code(removeprefix_step_code),
            "Error Output": trim_long_string(last_try_exec_output),
            "Instructions": {
                "IMPORTANT": [
    ...
    def gen_step_code(
        ...
        # Use diff prompt for debug
        if last_try_step_code and last_try_exec_output:
            return self._gen_step_code_with_diff(
                step_plan, prev_steps_code, data_analysis, based_code,
                last_try_step_code, last_try_exec_output, retry
            )
    ...
    def _gen_step_code_with_diff(
        self,
        step_plan: dict,
        prev_steps_code: str,
        data_analysis: Optional[str],
        based_code: Optional[str],
        last_try_step_code: str,
        last_try_exec_output: str,
        retry: int
    ):
        """Generate step code using diff format for debugging"""
        
        prompt = self._get_diff_prompt(
            step_plan, prev_steps_code, last_try_step_code, last_try_exec_output
        )
        ...
        for _ in range(retry):
            ...
            # Try to extract diff blocks from the response
            diff_blocks = extract_diff_blocks(response)
            if diff_blocks:
                # Apply the diff blocks to the last try step code
                removeprefix_step_code = last_try_step_code.removeprefix(f"{prev_steps_code}\n") if prev_steps_code else last_try_step_code
                modified_step_code = apply_diff_to_code(removeprefix_step_code, diff_blocks)
            
                logger.info(f"=== MODIFIED CODE AFTER DIFF ===")
                logger.info(f"{modified_step_code}")
                logger.info(f"=== END OF MODIFIED CODE ===")
            
                # Combine the modified step code with previous steps code
                code = f"{prev_steps_code}\n{modified_step_code}" if prev_steps_code else modified_step_code
                
                try:
                    ast.parse(code)
                    logger.info(f"Diff-based code generation succeeded. Applied {len(diff_blocks)} changes.")
                    return modified_step_code, code
                except Exception as e:
                    logger.info(f"Diff-based code generation failed: {e}")
                    continue
            else:
                logger.info("No valid diff blocks found in response")
        
        logger.info("Diff-based code generation failed, falling back to regular generation")
        return None, None
    ...
    def step(
        ...
        if not isinstance(plan_list, list) or not plan_list:
            logger.error("Plan decomposition failed. Marking node as buggy.")
            self.reset()
            exec_result = ExecutionResult(
                term_out=["Plan decomposition failed"],
                exec_time=0.0,
                exc_type="RuntimeError",
                exc_info={"message": "Plan decomposition failed", "phase": "plan_decomposition"},
                exc_stack=[],
            )
            return "", exec_result
